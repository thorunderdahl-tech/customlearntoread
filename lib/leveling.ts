...
export const LEVELS: Level[] = [
  {
    ...
    promptRules:
      "Exactly ONE tiny sentence or label per page, 1-3 words (e.g. 'A cat.', 'Sam hops.', 'Big dog!'). Vocabulary: the child's name, CVC words, and the most common sight words (a, the, I, my, go, see, is). One strong repeating pattern through the book. Present tense. No commas, no dialogue.",
  },
  {
    id: "beginner",
...
export function checkStory(draft: StoryDraft, level: Level): CheckResult {
  ...
  draft.pages?.forEach((p) => {
    ...
    const nonName = ws.filter((w) => norm(w) !== nameNorm);
    if (nonName.length) {
