...
The attached image is the approved MASTER CAST SHEET for a children's book. Redraw EXACTLY ONE character from it — character number ${position} counting left to right among the figures standing beside the hero child: ${memberDesc}.

Solo character reference sheet (turnaround): that ONE character drawn TWICE, side by side on a plain soft cream background — full-body FRONT view and full-body BACK view, neutral happy pose. COPY the character from the master sheet with zero changes: identical face, hair color/texture and EXACT hair length, skin tone, and every garment with its exact colors (or species, coloring, markings). The BACK view shows the true back of the hair and the PLAIN back of the top. NO other character may appear — exactly one character, two views. No text or logos.`;
}

/** One interior/cover page. `scene` is the (ideally expanded) art direction.
 * `editPrevious` = the LAST attached reference image is the page's previous
 * version and the director note should be applied as an EDIT to it, not a
 * from-scratch regeneration — far more reliable for targeted changes.
 * `soloRefCount` = how many solo cast reference sheets are attached
 * immediately AFTER the master sheet (0 = legacy layout, sheet + anchors). */
export function pagePrompt(scene: string, characterDescription: string, cast?: string, directorNote?: string, fixNotes?: string, editPrevious = false, soloRefCount = 0): string {
  const companionLine = cast
  ...
  const directorLine = directorNote
    ...
    : "";
  const editLine = editPrevious
    ? `\n\nEDIT MODE — the LAST attached image is the PREVIOUS VERSION of THIS page. Reproduce it faithfully — the same composition, camera angle, characters, poses, palette and background — changing ONLY what the ART DIRECTOR'S INSTRUCTION requires. Anything the instruction does not name must stay identical to the previous version. If the instruction removes something, leave natural background in its place — never substitute a replacement.`
    : "";
  const refMap = soloRefCount > 0
    ? `\n\nREFERENCE MAP — the attached images, in order:\n- Image 1: the MASTER CAST SHEET (the hero child, front and back view${cast ? ", plus every recurring cast member" : ""}).\n- Image${soloRefCount > 1 ? `s 2-${1 + soloRefCount}` : " 2"}: SOLO CHARACTER REFERENCES — one per recurring cast member, in the exact order the CAST LOCK lists them. When a cast member appears in this scene, copy them from THEIR solo reference (it is sharper than the master sheet).\n- ${editPrevious ? "Any image after those except the last: an approved page from this book (style anchor). The LAST image is this page's previous version (see EDIT MODE)." : "Every remaining image: an approved page from this same book (style anchor)."}`
    : "";
  const anchorsWhere = soloRefCount > 0
    ? (editPrevious ? "anchor images named in the REFERENCE MAP" : "the remaining attached images named in the REFERENCE MAP")
    : (editPrevious ? "attached images between the character sheet and the last image" : "any additional attached images");
  return `${STYLE}\n\nCHARACTER LOCK — the FIRST attached reference image is this book's character sheet (front view and back view of the same child). The child character MUST match the sheet EXACTLY:\n- face and eyes: identical\n- hair: identical color, texture, and the EXACT same length and style — never longer, shorter, curlier or straighter. Check the LANDMARK where the hair ends on the sheet (jaw, shoulders, mid-back or waist) and end it at the same landmark here.\n- skin tone: identical\n- outfit: the EXACT same garments, their colors and any graphic — NEVER change, swap or recolor any clothing between pages\nThe locked description: ${characterDescription}${companionLine}${refMap}\n\nCHARACTER COUNT: draw ONLY the characters this page's scene (and the art director's instruction) calls for — never add an extra child, adult or bystander to fill space, and never draw a person or creature who is not the hero, a listed cast member, or an explicitly-called-for background figure.\n\nOBJECT INTEGRITY: every object a character touches, holds, sits on, rides or stands on must be drawn COMPLETE and physically believable — a swing has a visible seat and both ropes/chains under the child, a bike has both wheels, a chair has its legs. Never leave a character floating on a missing or half-drawn object.\n\nSTYLE ANCHORS — ${anchorsWhere} are approved pages from this same book: match their rendering style, palette, level of detail AND lighting exactly so all pages look like one printed book. If an anchor page conflicts with the character sheet on any character detail, the CHARACTER SHEET wins.\n\nCLOTHING FROM BEHIND: if a character is turned so the back of their top is visible, draw it exactly as the sheet's BACK view shows — PLAIN fabric, no front graphic, logo or print copied onto the back. A front graphic on a back-facing body makes the head look reversed.\n\nSCENE FOR THIS PAGE: ${scene}${editLine}${directorLine}${fixNotes ? `\n\nFIX these problems from the previous attempt: ${fixNotes}` : ""}`;
}
...
1. CAST IDENTITY (most important): check EVERY character in the image — the hero AND every recurring friend, sibling or pet — against their locked description${styleRef ? " and the reference sheet" : ""}. Compare attribute by attribute and garment by garment: hair color, hair texture, hair LENGTH, eye color, SKIN TONE, glasses/accessories, then EACH garment (top, its color, any graphic; bottoms and color; shoes and color), species/markings/collar. HAIR-LENGTH LANDMARK TEST (do this explicitly for every character): decide which body landmark the hair reaches on the reference sheet — jaw, shoulders, mid-back, or waist — then decide which landmark it reaches in the page image. A different landmark is a HARD FAIL; say e.g. "hero's hair is waist-length, sheet shows shoulder-length". Also fail hair that is clearly bigger/more voluminous than the sheet. A character whose skin tone, hair, or ANY garment differs from the sheet — even slightly, even in the background — is a HARD FAIL. Name which character drifted and which attribute. Also check the CAST ROSTER itself: a prominent character in the image who matches NO locked description (an invented extra child, adult or creature) is a HARD FAIL — name them. If a character is shown from behind, the back of their top must be PLAIN fabric${styleRef ? " (match the sheet's back view if it has one)" : ""} — a front graphic, logo or print appearing on the BACK (which makes the head look put on backwards) is a HARD FAIL.
2. ${styleRef ? "STYLE & LIGHTING: does the rendering style (medium, palette, line treatment) match the reference sheet closely enough that both could be pages of the same printed book? And is the lighting the book's warm late-afternoon golden-hour — NOT flat midday, a cool/blue-sky palette, overcast grey or night? A page whose lighting or palette temperature clearly differs from the book's warm golden-hour look is a fail." : "STYLE & LIGHTING: warm hand-illustrated picture-book style with warm golden-hour light — no 3D/CGI, no anime, no photorealism, no flat midday or cool/blue palette?"}
...
6. OBJECT & PHYSICS COHERENCE: for EVERY object a character is interacting with — sitting on, holding, riding, climbing, standing on — verify the object is COMPLETE and the interaction is physically possible. A child "on a swing" must have a visible SEAT under them and ropes/chains that connect to it; a held ball must be in the hand; a ridden bike has both wheels; feet on a slide touch the slide. A character floating on a missing, incomplete or detached object is a HARD FAIL — name the object and what's missing.
7. COMPOSITION: are the subject's face, hands and every story-critical object fully inside the UPPER TWO-THIRDS of the frame, with the bottom of the frame simple background only, and nothing important within ~5% of any edge? (The reading-text band covers the bottom of the page and print trimming crops the edges.) ALSO: the scene itself must reach every edge of the frame — a baked-in solid/blank strip, cream or white band, letterbox or border at any edge (instead of continued scene like grass, floor or sky) is a fail; the real text band is composited later.
8. Is there ANY text, lettering, numbers or watermark in the image?
9. Anything inappropriate or scary for ages 3-7?
10. Expressions: do the characters look happy/warm? Flag any unintended angry, sad, scared or distressed face that the story text does NOT call for (the default should be happy).
11. If more than one child appears, do they look like SAME-AGE peers? Flag it if any child looks clearly older or younger than the others.
