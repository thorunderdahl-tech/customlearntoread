...
export default function SuccessPage({
  ...
  searchParams: { rla?: string; pid?: string; oid?: string; addon?: string; ff?: string };
}) {
  const readAlong = searchParams?.rla === "1";
  const addonDone = searchParams?.addon === "done";
  const friendsOrder = searchParams?.ff === "1"; // friends & family code — no payment, no receipt email
  return (
    <div className="center-narrow">
      ...
      {friendsOrder ? (
        <p>
          Thanks so much — your friends &amp; family code worked, so there&apos;s
          nothing to pay. We&apos;ve got everything we need and we&apos;ll get
          started on your book right away.
        </p>
      ) : (
        <p>
      ...
      {!addonDone && !friendsOrder && searchParams?.pid && (
      ...
      <p>
        ...
        If you signed up for the Quarterly Reading Club, your first set will
        start production immediately and ship as soon as it&apos;s ready.
      </p>
      <h2>Need to change something?</h2>
      {friendsOrder ? (
        <p>
          Email us at the address on our contact page — we can update names,
          spelling, themes, or shipping info before printing begins.
        </p>
      ) : (
        <p>
