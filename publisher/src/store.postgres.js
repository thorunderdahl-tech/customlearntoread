...
-- Forward-compat for tables created before retry support was added.
ALTER TABLE calendar_entries ADD COLUMN IF NOT EXISTS attempts INT NOT NULL DEFAULT 0;
ALTER TABLE calendar_entries ADD COLUMN IF NOT EXISTS next_attempt_at TIMESTAMPTZ;
...
function rowToEntry(r) {
  ...
  if (r.attempts) entry.attempts = r.attempts;
  if (r.next_attempt_at) entry.nextAttemptAt = r.next_attempt_at.toISOString();
  return entry;
}
...
async function updateEntryStatus(id, status, meta = {}) {
  await query(
    ...
    [
      ...
      meta.remoteId || null,
      meta.error || null,
      meta.attempts === undefined ? null : meta.attempts,
...
async function upsertContent(content) {
  await query(
    ...
    [content.id, content.caption || null, content.mediaUrl || null, content.mediaType || null]
  );
...
async function upsertEntry(entry) {
  await query(
    ...
    [entry.id, entry.channel, entry.contentId || null, entry.publishAt || null, entry.status || 'scheduled', entry.theme || null]
  );
  const { rows } = await query('SELECT * FROM calendar_entries WHERE id = $1', [entry.id]);
...
async function getLog(limit = 500) {
  const { rows } = await query(
    'SELECT * FROM post_log ORDER BY id DESC LIMIT $1',
    [limit]
  );
  return rows.map((r) => ({
    id: r.id,
